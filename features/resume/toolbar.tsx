...
export const StudioToolbar: React.FC<StudioToolbarProps> = ({
  ...
  return (
    <header className="h-14 bg-white border-b border-[#E2E8F0] px-3 sm:px-6 flex items-center justify-between shadow-xs z-30 shrink-0 select-none">
      {/* Left: Back Link & Resume Title */}
      <div className="flex items-center gap-3">
        <Link
          href="/"
          className="flex items-center gap-1.5 text-xs font-semibold text-[#64748B] hover:text-[#0F172A] p-1.5 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16A34A] transition-colors"
        ...
        <div className="flex items-center gap-2">
          <RolezenLogo size={22} />
          <span className="font-bold text-xs sm:text-sm text-[#0F172A] truncate max-w-[140px] sm:max-w-[220px]">
            Senior Staff Architect Resume
          </span>
      ...
      <div className="hidden lg:flex items-center gap-4">
        ...
        {/* Zoom Scale Controls */}
        <div className="flex items-center gap-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl px-2 py-1 text-xs text-[#0F172A] font-semibold">
          <button
            type="button"
      ...
      {/* Right: Inspector Triggers & Export */}
      <div className="flex items-center gap-2 sm:gap-3">
        <Button
          variant="outline"
          size="sm"
          leftIcon={<ShieldCheck className="h-4 w-4 text-[#16A34A]" />}
          onClick={() => onOpenInspector('ats')}
          className="text-xs px-2.5 sm:px-3 h-9"
        >
          <span>ATS: <strong className="text-[#16A34A]">{atsScore}</strong></span>
        </Button>
