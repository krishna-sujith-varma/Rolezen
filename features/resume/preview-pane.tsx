...
export const PreviewPane: React.FC<PreviewPaneProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#E2E8F0]/40 overflow-y-auto p-4 sm:p-8 lg:p-12 items-center justify-start select-none">
      ...
      >
        ...
        <div
          className={cn(
            'w-[794px] min-h-[1123px] bg-white rounded-sm shadow-2xl shadow-slate-900/15 border border-[#E2E8F0] p-12 space-y-6 text-[#0F172A] font-sans'
          )}
        >
          {/* Header Section */}
              ...
              {experiences.map((exp) => (
                <div key={exp.id} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs font-bold text-[#0F172A]">
                    ...
                    <span className="text-[#64748B] font-normal">{exp.startDate} — {exp.endDate}</span>
                  </div>
              ...
              {education.map((edu) => (
                <div key={edu.id} className="flex items-center justify-between text-xs">
                  <div>
                    ...
                    <span className="text-[#475569]">{edu.degree} in {edu.fieldOfStudy}</span>
                  </div>
                  <span className="text-[#64748B]">{edu.startDate} — {edu.endDate}</span>
                </div>
              ))}
            </div>
          </div>
